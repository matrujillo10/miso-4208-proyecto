...
const getBDTData = function(id) {
    ...
    return new Promise(function(resolve, reject) {
        c1.query(`
                select feature, steps, browser, browser_version, url, test_strategy_execution_id
                from public.app_bdttestexecution a 
                inner join public.app_bdttest b on a.bdt_test_id = b.id
            ...
            if (err) {
                ...
                data = res.rows[0]
                data.feature = host + data.feature
                data.steps = host + data.steps
                console.log(data)
                c1.end()
                const c2 = new Client({
                    connectionString: connectionString,
                ...
                c2.connect();
                c2.query(`update public.app_bdttestexecution set status = 'PROCESANDO' where id = $1::int`, [id], (err, res) => {
                    if (err) {
                        console.log(err.stack)
                        c2.end()
                        reject('error')

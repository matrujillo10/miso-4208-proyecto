...
const getE2EData = function(id) {
    ...
    return new Promise(function(resolve, reject) {
        c1.query(`
                select zip, spec, url, test_strategy_execution_id
                from public.app_e2etestexecution a 
                inner join public.app_e2etest b on a.e2e_test_id = b.id
            ...
            if (err) {
                ...
                c2.connect();
                c2.query(`update public.app_e2etestexecution set status = 'PROCESANDO' where id = $1::int`, [id], (err, res) => {
                    if (err) {
                        console.log(err.stack)
                        c2.end()
                        reject('error')

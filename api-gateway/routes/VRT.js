...
router.post('/', function(req, res, next) {
  if (req.files && req.fields.name && req.fields.description && req.fields.roundID && req.fields.spec) {
    let czip = req.files.cypress;
    let izip = req.files.images;
    var id = uuid();
    if (!czip.type.includes('zip') || Array.isArray(czip) || !izip.type.includes('zip') || Array.isArray(izip)) {
      next(createError(400, "The files uploaded are not ok. We only accept two zip file."));
    } else {
      uploadFileToStorage(czip).then(data1 => {
        uploadFileToStorage(izip).then(data2 => {
          db.collection(testType).doc(id).set({
            name: req.fields.name,
            description: req.fields.description,
            spec: req.fields.spec,
            cypress_zip: data1.url,
            images_zip: data2.url
          }).then(ans => {
            res.send({
              testID: id
            });
